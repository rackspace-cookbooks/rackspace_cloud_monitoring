...
describe 'rackspace_cloud_monitoring_check_test::* on Centos 6.5' do
  ...
  context 'Network check' do
    ...
    context 'rackspace_cloud_monitoring_check with missing target attribute' do
      cached(:chef_run) do
        ChefSpec::SoloRunner.new(CENTOS_CHECK_OPTS) do |node|
          node_resources(node)
          node.set['rackspace_cloud_monitoring_check_test']['type'] = 'agent.network'
  ...
  context 'Disk check' do
    ...
    context 'rackspace_cloud_monitoring_check for disk with missing target attribute' do
      cached(:chef_run) do
        ChefSpec::SoloRunner.new(CENTOS_CHECK_OPTS) do |node|
          node_resources(node)
