shared_examples_for 'rackspace monitoring agent set up' do |platform|
  it 'calls rackspace_cloud_monitoring_service resource' do
    expect(chef_run).to create_rackspace_cloud_monitoring_service('default')
    expect(chef_run).to start_rackspace_cloud_monitoring_service('default')
  end
